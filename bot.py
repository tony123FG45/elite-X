...
async def verify_card(card_data: str, cookie: str) -> dict:
    """
    Verifica una sola tarjeta contra la API.
    card_data: "4532640527811643|12|2025|123"
    """
    data = {
        "card": card_data,
        "cookies": cookie
    ...
    try:
        ...
        logger.error(f"Error al verificar tarjeta {card_data}: {e}")
        return {
            "status": "⚠️ Error",
...
async def mx_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Obtener el texto completo del mensaje (incluyendo argumentos si los hay)
    text = ""
    
    # Si hay argumentos, úsalos
    if context.args:
        text = " ".join(context.args)
    elif msg.text:
        # Si no hay argumentos, toma todo el texto
        text = msg.text
    
    # Eliminar el comando /mx si está presente
    if text.startswith('/mx'):
        text = text[3:].strip() # Elimina '/mx' y espacios
    
    # Si el texto está vacío después de quitar el comando, usar respuesta si existe
    ...
    # Limpiar y separar las tarjetas
    ...
    for line in lines:
        ...
        if not line:
            continue
        # Asegurar formato: numero|mes|año|cvv
        parts = line.split('|')
        if len(parts) == 4:
            cards_to_check.append(f"{parts[0]}|{parts[1]}|{parts[2]}|{parts[3]}")
        else:
            # Intentar arreglar formato si hay espacios
...
async def refe_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    formatted_refe = (
        f"🔥 <b>REFERENCIA ENVIADA</b>\n\n"
        f"👤 <b>Usuario:</b> {user.first_name} @{user.username if user.username else 'Sin username'}\n"
